...
void ChatPage::AppendChatMsg(std::shared_ptr<TextChatData> msg)
{
    ...
    if (msg->_from_uid == self_info->_uid) {
        ...
        auto icon = self_info->_icon;
        QPixmap pixmap(icon);
        if (pixmap.isNull()) {
            pixmap = QPixmap(heads[self_info->_uid % heads.size()]);
        }
        ...
        auto f_icon = friend_info->_icon;
        QPixmap fpixmap(f_icon);
        if (fpixmap.isNull()) {
            fpixmap = QPixmap(heads[friend_info->_uid % heads.size()]);
        }
        pChatItem->setUserIcon(fpixmap);
        QWidget* pBubble = nullptr;
        pBubble = new TextBubble(role, msg->_msg_content);
        pChatItem->setWidget(pBubble);
        ui->chat_data_list->appendChatItem(pChatItem);
...
void ChatPage::on_send_btn_clicked()
{
    ...
    for(int i=0; i<msgList.size(); ++i)
    {
        ...
        QPixmap sendPixmap(userIcon);
        if (sendPixmap.isNull()) {
            sendPixmap = QPixmap(heads[user_info->_uid % heads.size()]);
        }
        pChatItem->setUserIcon(sendPixmap);
        QWidget *pBubble = nullptr;
